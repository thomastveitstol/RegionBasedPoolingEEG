...
from src.data.data_generators.data_gen import PlainDataGenerator, zero_fill_input, ZeroFillDataGenerator
...
def main() -> None:
    ...
    model_name = f"FixedChannels_ZeroFill_{classifier_name}"
    ...
    for fold in range(num_folds):
        ...
        train_subjects = tuple(item for tuple_item in train_subjects for item in tuple_item)  # unpacking tuple
        ...
        val_gen = ZeroFillDataGenerator(subjects=val_subjects, dataset=dataset, seq_length=seq_length,
                                        time_series_start=10000, target="sex")
        ...
        test_gen = ZeroFillDataGenerator(subjects=test_subjects, dataset=dataset, seq_length=seq_length,
                                         time_series_start=10000, target="sex")
        ...
        for i, (channel_system_name, removed_channels) in enumerate(illegal_channels.items()):
            print(f"\t({i + 1}/{num_val_channel_systems+1})\t Zero filling: {channel_system_name}")
            # Zero-fill
            channel_indices = channel_names_to_indices(channel_names=removed_channels,
                                                       channel_name_to_index=channel_name_to_index)
            zero_filled_x = zero_fill_input(val_gen.x, channel_indices=channel_indices)

            # Add to val generator
            val_gen.x_interpolated[channel_system_name] = zero_filled_x
        ...
        for i, (channel_system_name, removed_channels) in enumerate(illegal_channels.items()):
            ...
            channel_indices = channel_names_to_indices(channel_names=removed_channels,
                                                       channel_name_to_index=channel_name_to_index)
            zero_filled_x = zero_fill_input(test_gen.x, channel_indices=channel_indices)

            # Add to val generator
            test_gen.x_interpolated[channel_system_name] = zero_filled_x

        # Test
        print("Testing model on test set...")
        model.eval()
        model.test_model(loader=test_loader, histories=test_histories, device=device,
