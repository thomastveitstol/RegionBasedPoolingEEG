...
from src.models.modules.bins.regions_to_bins import Regions2Bins
from src.utils import ChGroup


class Regions2BinsClassifierM1(MainModelBase):
    ...
    def __init__(self,
        ...
        # ---------------------------------------
        self._regions_2_bins_module = Regions2Bins(channel_splits=channel_splits, paths=paths,
        ...
        # classification
        # ---------------------------------------
        self._bins_classifier = MTSClassifier(classifier_name=mts_classifier_name, **mts_classifier_hyperparams)
    ...
    def generate_model(cls,
        ...
        Args:
            ...
            candidate_region_splits: Which region splits should be considered for splitting the EEG cap into regions
            pooling_methods: name of group pooling modules
            ...
            batch_norm: To apply batch norm after pooling (True) or not (False). Defaults to False
        ...
        regions_2_bins_module = Regions2Bins.generate_module(
            pooling_methods=pooling_methods, channel_systems=channel_systems,
            channel_split_hyperparams=channel_split_hyperparams, nodes=nodes, min_nodes=min_nodes,
            pooling_hyperparams=pooling_hyperparams, num_channel_splits=num_channel_splits,
        ...
        return cls(channel_splits=regions_2_bins_module.channel_splits, paths=regions_2_bins_module.paths,
                   pooling_methods=pooling_methods,
                   mts_classifier_name=stacked_bins_classifier_name,
                   mts_classifier_hyperparams=bins_classifier.hyperparameters,
    ...
    def pre_compute(self, x: torch.Tensor, channel_system_name: str,
                    channel_name_to_index: Dict[str, int], batch_size: int,
                    to_cpu: bool = True) -> Tuple[Tuple[torch.Tensor, ...], ...]:
        """Method for pre-computing features to be used by the attention module of the Regions2Bins object"""
        return self._regions_2_bins_module.pre_compute_batch(x=x, channel_system_name=channel_system_name,
                                                             channel_name_to_index=channel_name_to_index,
                                                             batch_size=batch_size, to_cpu=to_cpu)
    ...
    def test_model(self, *, loader: DataLoader, channel_systems: Tuple[BaseChannelSystem, ...],
        ...
        Args:
            ...
            activation_function: Activation function to use for predictions
        ...
        with torch.no_grad():
            for data, targets, pre_computed in loader:
                x = data.to(device)
                y = targets.to(device)
                precomputed_features = pre_computed

                # Compute validation metrics for all channel systems
                for channel_system in channel_systems:
                    # Predict
                    if precomputed_features is None or channel_system.name not in precomputed_features:
                        val_precomputed = None
                    else:
                        val_precomputed = precomputed_features[channel_system.name]
                    y_pred = self(x=x, precomputed=val_precomputed,
    ...
    def fit_model(self, *, train_loader: DataLoader, val_loader: DataLoader, train_history: Histories,
        ...
        for epoch in range(num_epochs):
            ...
            for data, targets, pre_computed in train_loader:
                ...
                # Forward pass
                if precomputed_features is None or train_channel_system.name not in precomputed_features:
                    precomputed = None
                else:
                    precomputed = precomputed_features[train_channel_system.name]
                scores = self(x, precomputed=precomputed,
                              channel_system_name=train_channel_system.name,
                              channel_name_to_index=channel_name_to_index)
            ...
            with torch.no_grad():
                for val_data, val_targets, val_pre_computed in val_loader:
                    ...
                    val_precomputed_features = val_pre_computed

                    # Compute validation metrics for all channel systems
                    for channel_system in val_channel_systems:
                        # Predict
                        if val_precomputed_features is None or channel_system.name not in val_precomputed_features:
                            val_precomputed = None
                        else:
                            val_precomputed = val_precomputed_features[channel_system.name]
                        y_pred = self(x=x_val, precomputed=val_precomputed,
                                      channel_system_name=channel_system.name,
                                      channel_name_to_index=channel_system.channel_name_to_index())
                        if activation_function is not None:
                            y_pred = activation_function(y_pred)
    ...
    @classmethod
    def from_disk(cls, path: str) -> 'Regions2BinsClassifierM1':
        """Method for loading model from disk"""
        # Make objects
        regions_2_bins = Regions2Bins.from_disk(path=os.path.join(path, "regions_2_bins.pt"))
        mts_classifier = MTSClassifier.from_disk(path=os.path.join(path, "mts_classifier.pt"))

        # ----------------------
        # Initialise model
        # ----------------------
        model = cls(channel_splits=regions_2_bins.channel_splits,
                    paths=regions_2_bins.paths,
                    pooling_methods=regions_2_bins.pooling_methods,
                    pooling_hyperparams=regions_2_bins.pooling_hyperparams,
                    mts_classifier_name=mts_classifier.name,
                    mts_classifier_hyperparams=mts_classifier.hyperparameters)
